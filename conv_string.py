...
if __name__ == "__main__":
    ...
    # 引数の追加
    parser.add_argument("-e", "--encode", help="encode sentences",nargs='*')
    parser.add_argument("-d","--decode",help="decode sentences",nargs='*')
    ...
    parser.add_argument("-i","--input",help="input sentences",action="store_true")
    ...
    if args.encode == [] or args.decode == []:
        if args.input:
            ...
            if args.decode == []:
                st = "decode"
            print("Input sentences to {}. If you want to break this mode, you can use Ctrl^C.".format(st))
            while True:
                try:
                    s = input(">>>")
            ...
            print("ArgumentError: You must type -e or -d arguments.")
            sys.exit()
    if args.decode:
        if len(args.encode) < 2:
            print("ArgumentError:You must type input-file and output-file!")
